...
def main():
  if len(sys.argv)!=3:
    print("Usage: json_to_lua_weaponinfo.py weaponInfo.json out_dir", file=sys.stderr); sys.exit(2)
  ...
  for name,_ in BUCKETS + [("misc", set())]:
    lua = "return " + to_lua(buckets.get(name, {})) + "\n"
    fn = os.path.join(outdir, f"WeaponInfo_{name}.lua")
    with open(fn,"w",encoding="utf-8") as f: f.write("-- auto-generated\n" + lua)
    names[name] = f"Module:Game/WeaponInfo_{name}"

  lines=[]
  for name, letters in BUCKETS:
    for ch in sorted(letters):
      lines.append(f'  ["{ch}"] = "{names[name]}",')
  lines.append(f'  misc = "{names["misc"]}",')
